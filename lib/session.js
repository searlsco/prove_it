...
// --- Cancel sentinel (file-based, session-scoped) ---

function cancelSentinelPath (sessionId) {
  return path.join(getProveItDir(), 'sessions', sessionId, 'cancel')
}

function writeCancelSentinel (sessionId) {
  if (!sessionId) return
  try {
    const p = cancelSentinelPath(sessionId)
    ensureDir(path.dirname(p))
...
function readCancelSentinel (sessionId) {
  if (!sessionId) return false
  try {
    return fs.existsSync(cancelSentinelPath(sessionId))
  } catch {
    return false
  }
}

function clearCancelSentinel (sessionId) {
  if (!sessionId) return
  try { fs.unlinkSync(cancelSentinelPath(sessionId)) } catch {}
}

// --- Disabled sentinel (file-based, session-scoped) ---
// Persists across the session so all subsequent hooks early-exit silently
// until the user runs `prove_it enable`.

function disabledSentinelPath (sessionId) {
  return path.join(getProveItDir(), 'sessions', sessionId, 'disabled')
}

function writeDisabledSentinel (sessionId) {
  if (!sessionId) return
  try {
    const p = disabledSentinelPath(sessionId)
    ensureDir(path.dirname(p))
    fs.writeFileSync(p, '', 'utf8')
  } catch {
    // Best-effort
...
function readDisabledSentinel (sessionId) {
  if (!sessionId) return false
  try {
    return fs.existsSync(disabledSentinelPath(sessionId))
  } catch {
    return false
  }
}

function clearDisabledSentinel (sessionId) {
  if (!sessionId) return
  try { fs.unlinkSync(disabledSentinelPath(sessionId)) } catch {}
}
...
function setSignal (sessionId, type, message) {
  ...
  saveSessionState(sessionId, SESSION_KEYS.SIGNAL, { type, message: message || null, at: Date.now() })
  return true
}
