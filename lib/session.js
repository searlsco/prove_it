...
function getConsecutiveUntestedEditCount (sessionId) {
  ...
  return loadSessionState(sessionId, 'consecutiveUntestedEditCount') || 0
}

function incrementConsecutiveUntestedEditCount (sessionId) {
  if (!sessionId) return
  const current = getConsecutiveUntestedEditCount(sessionId)
  saveSessionState(sessionId, 'consecutiveUntestedEditCount', current + 1)
}

function resetConsecutiveUntestedEditCount (sessionId) {
  if (!sessionId) return
  saveSessionState(sessionId, 'consecutiveUntestedEditCount', 0)
}
...
function setPhase (sessionId, phase) {
  ...
  saveSessionState(sessionId, 'phase', { phase, at: Date.now() })
  return true
}

function getPhase (sessionId) {
  const data = loadSessionState(sessionId, 'phase')
  return data?.phase || 'unknown'
}

function clearPhase (sessionId) {
  if (!sessionId) return
  saveSessionState(sessionId, 'phase', null)
}
