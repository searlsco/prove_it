...
function extractReviewText (result, jsonMode, reviewerEnv, rootDir, binary, timeout) {
  if (!jsonMode) {
    ...
    return { text: result.stdout.trim() || result.stderr.trim(), nudge: null }
  }
  ...
  if (parsed.subtype === 'error_max_turns' && parsed.sessionId) {
    ...
    if (nudgeResult.code === 0) {
      ...
      if (nudgeParsed && nudgeParsed.result) {
        return { text: nudgeParsed.result, nudge: { succeeded: true, numTurns } }
      }
      // Nudge JSON parse failed—use raw output
      return { text: nudgeResult.stdout.trim() || nudgeResult.stderr.trim(), nudge: { succeeded: true, numTurns } }
    }
    // Nudge failed—use whatever the original run produced
    return { text: parsed.result || result.stdout.trim() || result.stderr.trim(), nudge: { succeeded: false, numTurns } }
  }

  if (parsed.subtype === 'success' || parsed.result) {
    return { text: parsed.result, nudge: null }
  }
...
function runReviewer (rootDir, reviewerCfg, prompt) {
  ...
  const { text: responseText, nudge } = extractReviewText(result, jsonMode, reviewerEnv, rootDir, binary, timeout)
  ...
  if (!verdict.error) {
    return { available: true, ...verdict, responseText, nudge }
  }

  // Only attempt haiku classification when the reviewer exited gracefully.
  // When the nudge failed, the fallback text is likely raw JSON or garbage—
  // don't waste an API call trying to classify it.
  if (nudge && !nudge.succeeded) {
    return { available: true, error: 'nudge failed and reviewer produced no parseable verdict', responseText, nudge }
  }

  // Parser couldn't extract a verdict—ask haiku to classify
  const fallback = classifyVerdict(responseText)
  if (fallback.error) {
    return { available: true, error: fallback.error, responseText, nudge }
  }
  ...
  if (fallback.verdict === 'PASS') {
    return { available: true, pass: true, reason, responseText, nudge }
  }
  if (fallback.verdict === 'FAIL') {
    return { available: true, pass: false, reason, body, responseText, nudge }
  }
  // fallback.verdict === 'SKIP' (only remaining option)
  return { available: true, skip: true, reason, responseText, nudge }
}
