...
function resumeForVerdict (sessionId, binary, rootDir, timeout, reviewerEnv, runner, homeDir) {
  ...
  if (parsed) {
    ...
    return scrapeSessionVerdict(sessionId, homeDir) || null
  }
...
function runReviewer (rootDir, reviewerCfg, prompt) {
  ...
  // HACK: Last-resort session scrape (Claude Code --resume bug — see scrapeSessionVerdict)
  if (!responseText && extracted.sessionId) {
    responseText = scrapeSessionVerdict(extracted.sessionId)
  }
  if (!responseText) {
