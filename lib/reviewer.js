...
function extractReviewText (result, jsonMode, reviewerEnv, rootDir, binary, timeout) {
  if (!jsonMode) {
    ...
    return { text: result.stdout.trim() || result.stderr.trim(), finalTurn: null }
  }
  ...
  if (parsed.subtype === 'error_max_turns' && parsed.sessionId) {
    ...
    const finalTurnResult = tryRun(resumeCmd, {
      cwd: rootDir,
      timeout,
    ...
    if (finalTurnResult.code === 0) {
      ...
      if (finalTurnParsed && finalTurnParsed.result) {
        return { text: finalTurnParsed.result, finalTurn: { succeeded: true, numTurns } }
      }
      // Final turn JSON parse failed—use raw output
      return { text: finalTurnResult.stdout.trim() || finalTurnResult.stderr.trim(), finalTurn: { succeeded: true, numTurns } }
    }
    // Final turn failed—use whatever the original run produced
    return { text: parsed.result || result.stdout.trim() || result.stderr.trim(), finalTurn: { succeeded: false, numTurns } }
  }

  if (parsed.subtype === 'success' || parsed.result) {
    return { text: parsed.result, finalTurn: null }
  }
...
function runReviewer (rootDir, reviewerCfg, prompt) {
  ...
  const { text: responseText, finalTurn } = extractReviewText(result, jsonMode, reviewerEnv, rootDir, binary, timeout)
