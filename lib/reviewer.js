...
const WRAP_UP_PROMPT = `You have run out of turns. Provide your final verdict RIGHT NOW based on
 ...
 * When jsonMode is active, parses JSON and handles wrapUp-and-resume
 * for max_turns errors. Otherwise, returns raw text.
 */
function extractReviewText (result, jsonMode, reviewerEnv, rootDir, binary, timeout) {
  if (!jsonMode) {
    // Non-JSON path: claude -p writes to stderr; fall back when stdout is empty
    return { text: result.stdout.trim() || result.stderr.trim(), wrapUp: null }
  }
  ...
  if (parsed.subtype === 'error_max_turns' && parsed.sessionId) {
    // Wrap-up: resume the session and ask for a final verdict
    const numTurns = parsed.numTurns
    const resumeCmd = `${shellEscape(binary)} -p --resume ${shellEscape(parsed.sessionId)} --max-turns 1 --output-format json`
    const wrapUpResult = tryRun(resumeCmd, {
      cwd: rootDir,
      timeout,
    ...
    if (wrapUpResult.code === 0) {
      const wrapUpParsed = parseJsonOutput(wrapUpResult.stdout)
      if (wrapUpParsed && wrapUpParsed.result) {
        return { text: wrapUpParsed.result, wrapUp: { succeeded: true, numTurns } }
      }
      // Wrap-up JSON parse failed—use raw output
      return { text: wrapUpResult.stdout.trim() || wrapUpResult.stderr.trim(), wrapUp: { succeeded: true, numTurns } }
    }
    // Wrap-up failed—use whatever the original run produced
    return { text: parsed.result || result.stdout.trim() || result.stderr.trim(), wrapUp: { succeeded: false, numTurns } }
  }

  if (parsed.subtype === 'success' || parsed.result) {
    return { text: parsed.result, wrapUp: null }
  }
...
function runReviewer (rootDir, reviewerCfg, prompt) {
  ...
  const { text: responseText, wrapUp } = extractReviewText(result, jsonMode, reviewerEnv, rootDir, binary, timeout)
  ...
  if (!verdict.error) {
    return { available: true, ...verdict, responseText, wrapUp }
  }

  // Only attempt haiku classification when the reviewer exited gracefully.
  // When the wrapUp failed, the fallback text is likely raw JSON or garbage—
  // don't waste an API call trying to classify it.
  if (wrapUp && !wrapUp.succeeded) {
    return { available: true, error: 'wrap-up failed and reviewer produced no parseable verdict', responseText, wrapUp }
  }

  // Parser couldn't extract a verdict—ask haiku to classify
  const fallback = classifyVerdict(responseText)
  if (fallback.error) {
    return { available: true, error: fallback.error, responseText, wrapUp }
  }
  ...
  if (fallback.verdict === 'PASS') {
    return { available: true, pass: true, reason, responseText, wrapUp }
  }
  if (fallback.verdict === 'FAIL') {
    return { available: true, pass: false, reason, body, responseText, wrapUp }
  }
  // fallback.verdict === 'SKIP' (only remaining option)
  return { available: true, skip: true, reason, responseText, wrapUp }
}
