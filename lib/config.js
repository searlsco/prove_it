...
function hasCustomTests (config) {
  const tests = config?.tests
  if (!Array.isArray(tests) || tests.length === 0) return false
  const defaults = baseConfig().tests
  if (tests.length === defaults.length && tests.every((t, i) => t === defaults[i])) return false
  return true
}
