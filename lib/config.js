...
function addDefaultChecks (cfg) {
  // Add review:code_quality to Stop (churn-gated)
  ...
  if (stopEntry) {
    ...
    // Add review:test_coverage to Stop (net-churn-gated)
    stopEntry.tasks.push({
    ...
    // Add signal-gated code review (fires when agent signals "done")
    stopEntry.tasks.push({
      name: 'signal-review',
      type: 'agent',
      promptType: 'reference',
 ...
 * @param {boolean} options.defaultChecks - Include review:code_quality, review:test_coverage (default: true)
