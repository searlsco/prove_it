...
function isAncestor (dir, potentialAncestor, descendant) {
  if (!potentialAncestor || !descendant) return false
  if (potentialAncestor === descendant) return true
  const r = tryRun(`git -C ${shellEscape(dir)} merge-base --is-ancestor ${shellEscape(potentialAncestor)} ${shellEscape(descendant)}`, {})
  return r.code === 0
}
...
/**
 * Check if a `when` value (object or array of objects) contains a given key.
 */
function whenHasKey (when, key) {
  if (!when) return false
  if (Array.isArray(when)) return when.some(clause => clause[key] !== undefined)
  return when[key] !== undefined
}
 ...
 * the {{changes_since_last_run}} template variable regardless of whether
