...
function incrementGross (dir, delta) {
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    const oldSha = readRef(dir, '__gross')
    ...
    if (!oldSha) {
      ...
      updateRef(dir, '__gross', newSha)
      return
    }
    const cas = tryRun(`git -C ${shellEscape(dir)} update-ref refs/worktree/prove_it/__gross ${shellEscape(newSha)} ${shellEscape(oldSha)}`, {})
...
function grossChurnSince (dir, taskRefName) {
  ...
  const snapRef = `${taskRefName}.__snap`
...
function advanceGrossSnapshot (dir, taskRefName) {
  const global = readGrossCounter(dir)
  writeCounterRef(dir, `${taskRefName}.__snap`, global)
}
...
/**
 * Advance (or skip) the churn ref for a task based on pass/fail and hook event.
 *
 ...
 */
function advanceChurnRef (task, passed, hookEvent, rootDir, sources) {
  const hasNetChurn = task.when && task.when.linesWrittenSinceLastRun
  const hasGrossChurn = task.when && task.when.linesChangedSinceLastRun
  if (!hasNetChurn && !hasGrossChurn) return
