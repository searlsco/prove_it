...
function appendPlanBlock (filePath, { marker, block, position }) {
  ...
  } else if (position === 'before-steps') {
    // Insert before first numbered heading, fall back to before-verification behavior
    const firstStepPattern = /^#{2,3}\s+(?:Step\s+)?\d+[.:]/m
    const stepMatch = content.match(firstStepPattern)
    if (stepMatch) {
      const idx = stepMatch.index
