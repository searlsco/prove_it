...
function isEditingTool (toolName) {
  return EDITING_TOOLS.has(String(toolName || '').toLowerCase())
}
...
function recordToolResultObservation (facts, event, config, success) {
  ...
  if (paths.length === 0 && isEditingTool(event.toolName)) observation.missingTargetPath = true
  ...
  if (!isEditingTool(event.toolName) || paths.length === 0) return
