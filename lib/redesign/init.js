...
const { BUILT_IN_PROFILE, CONFIG_DIR, LOCAL_CONFIG_FILE, PROFILE_VERSION, SCHEMA_VERSION, loadEffectiveConfig, validateConfig } = require('./config')
const { activeGitWorkflowEvents, configureGitConfigHook, removeGitConfigHook } = require('../git_config_hooks')
...
function buildStrictProjectConfig (adapters) {
  ...
  const cfg = {
    ...
    profile: normalizedAdapters.length === 1
      ? (normalizedAdapters[0] === 'claude' ? 'claude' : 'pi')
      : 'strict',
...
function initStrictProject (repoRoot, options = {}) {
  ...
  if (options.gitHooks !== false && fs.existsSync(path.join(repoRoot, '.git')) && fs.existsSync(configPath)) {
    try {
      const effective = loadEffectiveConfig(repoRoot, { requireConfigFile: true })?.effective
      for (const event of activeGitWorkflowEvents(effective)) {
        const hookResult = configureGitConfigHook(repoRoot, event)
        const resultKey = event === 'pre-commit' ? 'preCommit' : 'prePush'
        result.gitConfigHooks[resultKey] = hookResult
        if (hookResult.configured) {
          manifest.git_hooks.push({
            owner: OWNER,
            name: hookResult.hook.name,
            event: hookResult.hook.event,
            command: hookResult.hook.command
          })
        } else if (hookResult.skipped) {
          result.skipped.push({ path: `git config hook.${event}`, reason: hookResult.reason })
        }
      }
    } catch (error) {
      result.skipped.push({ path: 'git config hooks', reason: error.message })
    }
  }

  if (manifest.artifacts.length > 0 || manifest.git_hooks.length > 0) {
...
function deinitStrictProject (repoRoot) {
  ...
  for (const gitHook of manifest.git_hooks || []) {
    if (!gitHook || gitHook.owner !== OWNER || typeof gitHook.event !== 'string') continue
    if (removeGitConfigHook(repoRoot, gitHook.event)) {
      result.removed.push(`git config hook.${gitHook.name}`)
    }
  }
