...
function askConflict (rl, opts) {
  ...
  return new Promise(resolve => {
    ...
    function prompt () {
      if (mergeApplied) {
        const mergeHint = defaultYes ? '[Ynd?]' : '[yNd?]'
        rl.question(`Accept merge: ${label}? ${mergeHint} `, answer => {
          ...
          _log('  Y - yes, accept the merged version')
          _log('  n - no, skip this file')
          _log('  d - show diff again')
          prompt()
        })
        return
      }
