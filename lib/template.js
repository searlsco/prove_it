...
const { gitHead, diffStatSinceRef, sanitizeRefName } = require('./git')
const { generateDiffsSince, loadSessionState, getFileEdits } = require('./session')
const { gitDiffFiles } = require('./git')
...
function makeResolvers (context) {
  ...
  return {
    ...
    session_diff: lazy('session_diff', () => {
      ...
      // Git-based fallback: scoped to files this session's tools touched
      const edits = getFileEdits(sessionId)
      if (!edits || edits.files.length === 0) return ''
      const baseHead = loadSessionState(sessionId, 'last_stop_head') ||
        loadSessionState(sessionId, 'git')?.head
      if (!baseHead) return ''
      const diff = gitDiffFiles(rootDir, baseHead, edits.files)
      if (!diff) return ''
    ...
    changes_since_last_review: lazy('changes_since_last_review', () => {
      const taskName = context.taskName
      if (!taskName) return ''
      const stat = diffStatSinceRef(rootDir, sanitizeRefName(taskName), context.sources)
      return stat || ''
    }),
    recently_edited_files: lazy('recently_edited_files', () => {
      const r = tryRun('git diff HEAD --name-only', { cwd: rootDir })
