...
const { gitHead, sanitizeRefName, readRef, updateRef, sourcePathspecs, findUntrackedSources, removeFromIndex, isGitRepo, isAncestor } = require('./git')
const { SESSION_KEYS, generateDiffsSince, loadSessionState } = require('./session')
...
const EDIT_VAR_DESCRIPTIONS = {
  edit_file_path: 'File path of the edited file (from tool_input)',
  edit_old_string: 'Original text replaced by Edit/MultiEdit',
  edit_new_string: 'Replacement text from Edit/MultiEdit',
  edit_content: 'Full file content from Write',
  edit_tool_name: 'Name of the tool that performed the edit (Edit, Write, etc.)'
}
...
const VAR_DESCRIPTIONS = {
  ...
  phase: 'Current session activity phase (unknown, plan, implement, refactor)',
  ...EDIT_VAR_DESCRIPTIONS
}

const SESSION_VARS = ['session_diff', 'session_id', 'signal_message', 'phase']
...
function resolveBaseline (rootDir, context) {
  ...
  if (context.taskName) {
    const refName = sanitizeRefName(context.taskName)
    // Floor stale refs to session start HEAD to avoid showing
    // changes from prior sessions
    if (context.sessionId) {
      const git = loadSessionState(context.sessionId, SESSION_KEYS.GIT)
      const existing = readRef(rootDir, refName)
      if (git?.head && existing && isAncestor(rootDir, existing, git.head)) {
        updateRef(rootDir, refName, git.head)
      }
    }
    const refSha = readRef(rootDir, refName)
    if (refSha) return refSha
    // Task has never run — use session start HEAD so the first run
    // sees all session changes. LAST_STOP_HEAD advances on every Stop,
    // which starves signal-gated tasks that only run occasionally.
    if (context.sessionId) {
      const git = loadSessionState(context.sessionId, SESSION_KEYS.GIT)
  ...
  if (context.sessionId) {
    const checkpoint = loadSessionState(context.sessionId, SESSION_KEYS.LAST_STOP_HEAD)
...
function makeResolvers (context) {
  ...
  return {
    ...
    session_diff: lazy('session_diff', () => {
      ...
      const lastSnapshotId = loadSessionState(sessionId, SESSION_KEYS.LAST_REVIEW_SNAPSHOT)
      const diffs = generateDiffsSince(sessionId, projectDir, lastSnapshotId, MAX_DIFF_CHARS)
    ...
    phase: lazy('phase', () => {
      const { getPhase } = require('./session')
      return getPhase(sessionId)
    }),
    ...
    edit_file_path: lazy('edit_file_path', () => toolInput?.file_path || toolInput?.notebook_path || ''),
    edit_old_string: lazy('edit_old_string', () => toolInput?.old_string || ''),
    edit_new_string: lazy('edit_new_string', () => toolInput?.new_string || ''),
    edit_content: lazy('edit_content', () => toolInput?.content || ''),
    edit_tool_name: lazy('edit_tool_name', () => context.toolName || ''),
