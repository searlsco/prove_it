...
function formatEntry (entry, maxWidth, opts) {
  ...
  let reason = reasonPrefix + (entry.reason || '').split('\n')[0]
  let showTag = !!hookTag
...
function formatVerbose (entry) {
  ...
  if (v.prompt != null) {
    ...
    if (v.backchannelContent) {
      box('appeal', v.backchannelContent)
    }
    if (v.notepadContent) {
...
module.exports = { monitor, findLatestSession, listSessions, monitorProject, findProjectLogFiles, projectHash, formatEntry, formatVerbose, formatTime, formatDuration, useColor, stripAnsi, visualWidth, normalizeHookTag, middleTruncatePath, truncateReason, progressiveTruncate, watchFile, shouldDisplay, displayStatusOf, findProveItProject }
