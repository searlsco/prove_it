...
const { getProveItDir, findProveItProject } = require('./config')
...
/**
 * Extract the developer's actual appeal text from backchannel content.
 * The backchannel template uses \n---\n separators; the developer writes
 * after the last one.
 */
function extractAppealText (backchannelContent) {
  if (!backchannelContent) return null
  const parts = backchannelContent.split('\n---\n')
...
function formatEntry (entry, maxWidth, opts) {
  ...
  if (rawStatus === 'RUNNING') {
    displayStatus = 'EXEC'
  } else if (rawStatus === 'SET') {
    displayStatus = 'DING'
  } else if (rawStatus.startsWith('ENFORCED:')) {
    displayStatus = rawStatus.slice('ENFORCED:'.length)
    reasonPrefix = '(Deferred) '
  }
  ...
  if (displayStatus === 'PLEA' && entry.verbose && entry.verbose.backchannelContent) {
    const appealText = extractAppealText(entry.verbose.backchannelContent)
    if (appealText) reasonText = appealText
  }
  let reason = reasonPrefix + reasonText.split('\n')[0]
  let showTag = !!hookTag
...
function formatVerbose (entry) {
  ...
  if (v.prompt != null) {
    ...
    if (v.notepadContent) {
      const roundLabel = v.notepadRound ? ` (round ${v.notepadRound})` : ''
      box(`continuation notes${roundLabel}`, v.notepadContent)
    }
  }

  // PLEA entry with backchannel content (no prompt/response)
  if (v.prompt == null && v.backchannelContent) {
    const appealText = extractAppealText(v.backchannelContent)
    box('appeal', appealText || '(template only — no response written)')
  }
...
/**
 * Compute the display status from a raw entry status.
 * RUNNING→EXEC, SET→DING, ENFORCED:*→*, everything else passes through.
 */
function displayStatusOf (rawStatus) {
  if (rawStatus === 'RUNNING') return 'EXEC'
  if (rawStatus === 'SET') return 'DING'
  if (rawStatus && rawStatus.startsWith('ENFORCED:')) return rawStatus.slice('ENFORCED:'.length)
  return rawStatus || '???'
}

/**
 * Determine whether an entry should be displayed given the current filter opts.
 *
 * Filters (all optional):
 *   statusFilter  – inclusion list of display statuses (uppercased)
 *   statusExclude – exclusion list of display statuses (uppercased)
 *   taskFilter    – inclusion list of reviewer names (lowercased)
 *   taskExclude   – exclusion list of reviewer names (lowercased)
 */
function shouldDisplay (entry, opts) {
  if (!opts) return true

  const status = displayStatusOf(entry.status)
  if (opts.statusFilter && !opts.statusFilter.includes(status)) return false
  if (opts.statusExclude && opts.statusExclude.includes(status)) return false

  const reviewer = (entry.reviewer || '').toLowerCase()
  if (opts.taskFilter && !opts.taskFilter.includes(reviewer)) return false
  if (opts.taskExclude && opts.taskExclude.includes(reviewer)) return false
  ...
  try {
    ...
    for (const line of chunk.split('\n')) {
      ...
      try {
        ...
        if (!shouldDisplay(entry, opts)) continue
...
/**
 * Set up stdin keypress listener so 'q' (or ctrl-c) quits the monitor.
 * Accepts an optional `input` stream for testing (defaults to process.stdin).
 * Returns a cleanup function, or null if stdin is not a TTY and no stream was provided.
 */
function setupQuitKey (onQuit, input) {
  const stream = input || process.stdin
  if (!input && !stream.isTTY) return null
...
function monitorSession (sessionId, opts) {
  ...
  console.log('watching for new entries\u2026 (q or ctrl-c to stop)')
...
function monitor (opts) {
  ...
  if (opts.statusExclude) displayOpts.statusExclude = opts.statusExclude
  if (opts.taskFilter) displayOpts.taskFilter = opts.taskFilter
  if (opts.taskExclude) displayOpts.taskExclude = opts.taskExclude
  ...
  if (!sessionId) {
    // Default: scope to current project if prove_it-enabled
    const projectDir = findProveItProject(process.cwd())
    if (projectDir) {
      return monitorProject(projectDir, Object.keys(displayOpts).length > 0 ? displayOpts : undefined)
    }
    console.error('Not in a prove_it-enabled project.\n')
    console.error('To monitor this project, run: prove_it init')
    console.error('Or use one of:')
    console.error('  prove_it monitor --all               Monitor all sessions')
    console.error('  prove_it monitor --project=/path     Monitor a specific project')
    console.error('  prove_it monitor <session-id>        Monitor a specific session')
    process.exit(1)
  }
...
module.exports = { monitor, findLatestSession, listSessions, monitorProject, findProjectLogFiles, projectHash, formatEntry, formatVerbose, formatTime, formatDuration, useColor, stripAnsi, visualWidth, normalizeHookTag, middleTruncatePath, truncateReason, progressiveTruncate, watchFile, shouldDisplay, displayStatusOf, findProveItProject, extractAppealText, setupQuitKey }
