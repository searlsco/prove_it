...
function runScriptCheck (check, context) {
  ...
  if (context.tests) hookInput.tests = context.tests
  if (context.testCommands) hookInput.testCommands = context.testCommands
  ...
  const output = truncateChars(combined, maxChars)
  ...
  // Write full output to a temp file so the agent can read it on demand
  let tempFilePath = ''
  if (combined) {
    const safeName = (check.name || 'unknown').replace(/[^a-zA-Z0-9_-]/g, '_')
    const fileName = `prove_it-${safeName}-${Date.now()}-${process.pid}.log`
    tempFilePath = path.join(os.tmpdir(), fileName)
    try { fs.writeFileSync(tempFilePath, combined, 'utf8') } catch {}
  }

  const tail = combined ? combined.slice(-1024) : '(no output)'
  const parts = [
    `${check.name}: ${command} failed (exit ${r.code}, ${(durationMs / 1000).toFixed(1)}s)`
  ]
  if (tempFilePath) parts.push(`Full output: ${tempFilePath}`)
  parts.push(`--- last 1 KB of output ---\n${tail}`)
