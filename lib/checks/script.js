...
/**
 * Resolve a builtin command name to its implementation path.
 * builtin:<name> maps to lib/checks/builtins.js exported function.
 */
function isBuiltin (command) {
  return command && command.startsWith('prove_it builtin:')
}

function getBuiltinName (command) {
  return command.replace('prove_it builtin:', '')
}
