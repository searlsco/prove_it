...
/**
 * review:commit_quality — Agent reviews staged diff for bugs/dead code/coverage.
 */
function reviewCommitQuality (check, context) {
  const { runAgentCheck } = require('./agent')
  const agentCheck = {
    name: check.name || 'commit-review',
    type: 'agent',
    command: 'claude -p',
  ...
  return runAgentCheck(agentCheck, context)
}

/**
 * review:test_coverage — Agent reviews session diffs for test coverage.
 */
function reviewCoverage (check, context) {
  if (!context.sessionId) {
    return {
      pass: false,
      reason: 'review:test_coverage requires a Claude Code session (session_id is missing).\nThis builtin must run in a Claude Code hook context (e.g. Stop), not from CLI or git hooks.',
      output: '',
      skipped: false
    }
  }

  // Check if there are any session diffs to review
  const { expandTemplate } = require('../template')
  const diffs = expandTemplate('{{session_diffs}}', context)
  if (!diffs.trim()) {
    return { pass: true, reason: 'no session diffs to review', output: '', skipped: true }
  }

  const { runAgentCheck } = require('./agent')
  const agentCheck = {
    name: check.name || 'coverage-review',
    type: 'agent',
    command: 'claude -p',
    timeout: check.timeout,
    prompt: 'Review the code changes below for test coverage adequacy.\n\nThe standard: if any changed line were reverted, would an existing test fail?\nIf not, a test is missing. Bug fixes and defensive guards especially need\nregression tests — they encode behavior that was previously wrong.\n\nDo not require tests for: comments, whitespace, log messages, or config-only changes.\n\nRespond with exactly one of:\n- PASS: <brief reasoning for why coverage is adequate>\n- FAIL: <what specific test is missing>\n\n{{session_diffs}}'
  }
  return runAgentCheck(agentCheck, context)
}
...
module.exports = {
  ...
  'review:commit_quality': reviewCommitQuality,
  'review:test_coverage': reviewCoverage,
  GATED_TOOLS,
  BASH_WRITE_PATTERNS
}
