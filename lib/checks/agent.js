...
function runAgentCheck (check, context) {
  ...
  if (!userPrompt || !userPrompt.trim()) {
    logReview(sessionId, projectDir, check.name, 'SKIP', 'empty prompt')
  ...
  if (check.ruleFile) {
    ...
    try {
      ...
      logReview(sessionId, projectDir, check.name, 'FAIL', detail)
...
${userPrompt}`
  ...
  const model = check.model || defaultModel(context.hookEvent, !!check.command)
  const reviewerCfg = { command, timeout, model }
