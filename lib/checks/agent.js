...
Do NOT use SKIP because you are unsure—if in doubt, render PASS or FAIL.${notepadInstruction}`
  ...
  const verbose = {
    ...
    numTurns: review.nudge?.numTurns || null,
  ...
  // Log nudge events so they're visible in prove_it monitor
  if (review.nudge?.succeeded) {
    const numTurns = review.nudge.numTurns != null ? review.nudge.numTurns : '?'
    log('TICK', `Told ${check.name} agent to wrap it up (used ${numTurns}/${maxAgentTurns} turns)`, { verbose })
  } else if (review.nudge && !review.nudge.succeeded) {
    const numTurns = review.nudge.numTurns != null ? review.nudge.numTurns : '?'
    log('BOOM', `Nudge failed for ${check.name} (used ${numTurns}/${maxAgentTurns} turns)`, { verbose })
  }
