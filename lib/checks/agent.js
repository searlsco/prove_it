...
const { backchannelDir, backchannelReadmePath, notepadDir, notepadFilePath } = require('../paths')
...
const { SESSION_KEYS, logReview, saveSessionState, loadSessionState } = require('../session')
const { DEFAULT_MODELS } = require('../defaults')
...
function getNotepadRound (sessionId, taskName) {
  if (!sessionId) return 0
  const rounds = loadSessionState(sessionId, SESSION_KEYS.NOTEPAD_ROUNDS) || {}
  return rounds[sanitizeTaskName(taskName)] || 0
}

function setNotepadRound (sessionId, taskName, value) {
  if (!sessionId) return
  const rounds = loadSessionState(sessionId, SESSION_KEYS.NOTEPAD_ROUNDS) || {}
  rounds[sanitizeTaskName(taskName)] = value
  saveSessionState(sessionId, SESSION_KEYS.NOTEPAD_ROUNDS, rounds)
}
...
function runAgentCheck (check, context) {
  ...
  function log (status, reason, extra) {
    if (check.quiet && status !== 'FAIL' && status !== 'BOOM' && status !== 'TICK') return
  ...
  const backchannelBlock = appealText
...
--- End Developer Backchannel ---\n`
  ...
  const currentRound = getNotepadRound(sessionId, check.name)

  const notepadBlock = notepadContent
    ? `\n--- Reviewer Continuation (round ${currentRound + 1} after ${currentRound} failure(s)) ---
A previous reviewer performed a primary review and left these notes:

${notepadContent.trimEnd()}

This is round ${currentRound + 1}. You MUST:
1. Verify each previous finding against the ACTUAL code. Use tools to read
   source files and trace data flow. Do NOT rely on code comments, variable
   names, or commit messages to confirm or dismiss a finding.
2. If a finding has been addressed, confirm with evidence (cite the fix).
3. If a finding has NOT been addressed in the code, it stands—re-flag it.
4. Check for NEW issues introduced by any remediation attempts.
5. Conduct a thorough review with the same rigor as a first-round review.
   The previous notes are context, not a shortcut.

You may NOT dismiss a previous finding without demonstrating—via traced code
paths and concrete references—that the underlying issue was resolved or that
the finding was factually wrong.
--- End Reviewer Continuation ---\n`
    : ''

  const notepadInstruction = sessionId
    ? `\nWhen you FAIL this review, you MUST include a structured continuation note
in your output after your verdict and findings. This note will be saved
automatically and used as context for the next review cycle.

After your FAIL verdict and developer-facing feedback, include a section
starting with "--- Continuation Note ---" that contains ALL of the following:
...
- Do not assume the next reviewer has any prior context beyond this note and
  the code itself.
- If you PASS the review, do NOT include a continuation note.`
    : ''
  ...
  const ruleBlock = rulesSection
    ...
    : ''
  const contextFilesBlock = reviewerContextFilesBlock(context.reviewerContextFiles)
...
${ruleBlock}${contextBlock}${backchannelBlock}${notepadBlock}
${userPrompt}
...
Do NOT use SKIP because you are unsure—if in doubt, render PASS or FAIL.${notepadInstruction}`
  ...
  const maxAgentTurns = check.maxAgentTurns || context.configMaxAgentTurns
  const tools = context.taskAllowedTools
  ...
  const reviewerCfg = {
    ...
    allowedTools,
    bypassPermissions: context.taskBypassPermissions
  }
  ...
  const verbose = {
    ...
    model: model || null,
    maxAgentTurns,
    numTurns: review.finalTurn?.numTurns ?? null,
    rawStdout: review.rawStdout != null ? review.rawStdout : null,
    rawStderr: review.rawStderr != null ? review.rawStderr : null,
  ...
  // Log final-turn events so they're visible in prove_it monitor
  if (review.finalTurn?.succeeded) {
    const numTurns = review.finalTurn.numTurns != null ? review.finalTurn.numTurns : '?'
    log('TICK', `${check.name} exceeded turn limit — final turn produced verdict (${numTurns}/${maxAgentTurns} turns)`, { verbose })
  } else if (review.finalTurn && !review.finalTurn.succeeded) {
    const numTurns = review.finalTurn.numTurns != null ? review.finalTurn.numTurns : '?'
    log('BOOM', `${check.name} exceeded turn limit — final turn failed (${numTurns}/${maxAgentTurns} turns)`, { verbose })
  }
  ...
  if (review.error) {
    // If the reviewer wrote a notepad before crashing, count it as a failure
    // round so the next reviewer gets accurate continuation context.
    const orphanedNotepad = readNotepad(rootDir, sessionId, check.name)
    if (orphanedNotepad && !notepadContent) {
      setNotepadRound(sessionId, check.name, currentRound + 1)
    }
    ...
    if (check.model) {
      const reason = `${check.name} crashed: ${review.error}\n\n` +
        ...
        'To unblock: ask the user to check the config, then re-signal.'
  ...
  if (review.skip) {
    ...
    setNotepadRound(sessionId, check.name, 0)
  ...
  if (review.pass === false) {
    ...
    // Dispatcher owns notepad bookkeeping — save the full review as continuation context
    const notepadContent = review.body
      ? `# ${check.name} Continuation Note\n\n${review.body}`
      : `# ${check.name} Continuation Note\n\n${review.reason}`
    writeNotepad(rootDir, sessionId, check.name, notepadContent)
    setNotepadRound(sessionId, check.name, currentRound + 1)
...
module.exports = { defaultModel, runAgentCheck, createBackchannel, cleanBackchannel, readBackchannel, writeNotepad, readNotepad, cleanNotepad, getNotepadRound, setNotepadRound }
