...
--- End Reviewer Continuation ---\n`
  ...
  const notepadInstruction = sessionId
    ? `\nIf you FAIL this review, write a continuation note for the next reviewer at:
${notepadFilePath(rootDir, sessionId, check.name)}
Include: what you found, how close the code is to passing, and what
specifically to check on the next round.`
    : ''
...
Do NOT use SKIP because you are unsure—if in doubt, render PASS or FAIL.${notepadInstruction}`
  ...
  if (review.pass === false) {
    ...
    // Notepad: increment round, write fallback if reviewer didn't write one.
    // Compare current notepad to pre-review snapshot: if changed, the reviewer
    // wrote it during execution and we preserve it; otherwise overwrite.
    saveSessionState(sessionId, roundKey, currentRound + 1)
    const postReviewNotepad = readNotepad(rootDir, sessionId, check.name)
    if (!postReviewNotepad || postReviewNotepad === notepadContent) {
      writeNotepad(rootDir, sessionId, check.name, fullReport)
    }
...
module.exports = { defaultModel, runAgentCheck, backchannelDir, backchannelReadmePath, createBackchannel, cleanBackchannel, notepadDir, notepadFilePath, writeNotepad, readNotepad, cleanNotepad }
