...
Do NOT use SKIP because you are unsure—if in doubt, render PASS or FAIL.${notepadInstruction}`
  ...
  const verbose = {
    ...
    numTurns: review.wrapUp?.numTurns || null,
  ...
  // Log wrap-up events so they're visible in prove_it monitor
  if (review.wrapUp?.succeeded) {
    const numTurns = review.wrapUp.numTurns != null ? review.wrapUp.numTurns : '?'
    log('TICK', `Told ${check.name} agent to wrap it up (used ${numTurns}/${maxAgentTurns} turns)`, { verbose })
  } else if (review.wrapUp && !review.wrapUp.succeeded) {
    const numTurns = review.wrapUp.numTurns != null ? review.wrapUp.numTurns : '?'
    log('BOOM', `Wrap-up failed for ${check.name} (used ${numTurns}/${maxAgentTurns} turns)`, { verbose })
  }
