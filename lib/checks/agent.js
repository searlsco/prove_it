...
const { DEFAULT_MODELS, DEFAULT_ALLOWED_TOOLS } = require('../defaults')
...
function getNotepadRound (sessionId, taskName) {
  ...
  const rounds = loadSessionState(sessionId, 'notepadRounds') || {}
...
function setNotepadRound (sessionId, taskName, value) {
  ...
  saveSessionState(sessionId, 'notepadRounds', rounds)
}
...
Do NOT use SKIP because you are unsure—if in doubt, render PASS or FAIL.${notepadInstruction}`
  ...
  const tools = context.taskAllowedTools || DEFAULT_ALLOWED_TOOLS
...
module.exports = { defaultModel, runAgentCheck, backchannelDir, backchannelReadmePath, createBackchannel, cleanBackchannel, readBackchannel, notepadDir, notepadFilePath, writeNotepad, readNotepad, cleanNotepad, getNotepadRound, setNotepadRound }
