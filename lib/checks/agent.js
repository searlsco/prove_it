...
function runAgentCheck (check, context) {
  ...
  // Wrap with format enforcement — encourages investigation before verdict
  const prompt = `You are a code reviewer. Investigate the changes described below.\nYou SHOULD use tools to read source files and check git history to verify your conclusions.\nThe diff is a starting point — if the code on disk and git history show things are fine, PASS.\n\nYour response MUST start with exactly one of:\nPASS: <brief reasoning>\nFAIL: <one-line reason>\n\nYou may include additional analysis after the verdict line.\n\n${userPrompt}`
