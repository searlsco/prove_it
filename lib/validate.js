...
const VALID_WHEN_KEYS = ['fileExists', 'envSet', 'envNotSet', 'variablesPresent', 'linesChangedSinceLastRun', 'linesWrittenSinceLastRun', 'sourcesModifiedSinceLastRun', 'toolsUsed', 'sourceFilesEdited']

const KNOWN_TOP_LEVEL_KEYS = [
  'configVersion', 'enabled', 'sources', 'format', 'taskEnv', 'hooks', 'runs', 'initSeed', 'fileEditingTools', 'model'
]
const KNOWN_HOOK_KEYS = [
  'type', 'event', 'tasks', 'matcher', 'triggers', 'source'
]
const KNOWN_TASK_KEYS = [
  'name', 'type', 'command', 'prompt', 'promptType', 'when', 'timeout', 'mtime', 'model', 'ruleFile', 'enabled', 'resetOnFail'
]
  ...
  if ('when' in task) {
    if (!task.when || typeof task.when !== 'object' || Array.isArray(task.when)) {
      ...
      for (const key of Object.keys(task.when)) {
        ...
        } else if (key === 'linesChangedSinceLastRun') {
          if (typeof task.when[key] !== 'number' || task.when[key] <= 0) {
            errors.push(`${prefix}.when.${key} must be a positive number`)
          }
        } else if (key === 'linesWrittenSinceLastRun') {
          if (typeof task.when[key] !== 'number' || task.when[key] <= 0) {
            errors.push(`${prefix}.when.${key} must be a positive number`)
          }
          if (hookType === 'git') {
            warnings.push(`${prefix} has "linesWrittenSinceLastRun" but hook type is "git" (this condition requires a Claude Code session for accumulation and will always be 0 in git hooks)`)
          }
