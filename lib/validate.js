...
const VALID_WHEN_KEYS = ['fileExists', 'envSet', 'envNotSet', 'variablesPresent']
...
function validateHookEntry (entry, idx, errors, warnings) {
  ...
  if (!('tasks' in entry)) {
    ...
    for (let j = 0; j < entry.tasks.length; j++) {
      validateTask(entry.tasks[j], idx, j, errors, warnings, entry.event)
    }
  }
...
function validateTask (task, hookIdx, taskIdx, errors, warnings, hookEvent) {
