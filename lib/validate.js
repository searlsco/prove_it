...
const VALID_WHEN_KEYS = ['fileExists', 'envSet', 'envNotSet', 'variablesPresent', 'linesChanged', 'linesWritten', 'sourcesModifiedSinceLastRun', 'toolsUsed', 'sourceFilesEdited', 'signal']
    ...
    if (!task.when || typeof task.when !== 'object' || Array.isArray(task.when)) {
      ...
      for (const key of Object.keys(task.when)) {
        if (!VALID_WHEN_KEYS.includes(key)) {
          ...
          if (typeof task.when[key] !== 'string' || !VALID_SIGNALS.includes(task.when[key])) {
            errors.push(`${prefix}.when.signal must be one of: ${VALID_SIGNALS.join(', ')}`)
          }
          if (hookType === 'git') {
            warnings.push(`${prefix} has "signal" but hook type is "git" (signals require a Claude Code session and have no effect in git hooks)`)
          }
          if (hookEvent === 'SessionStart') {
            warnings.push(`${prefix} has "signal" but hook event is "SessionStart" (signals are consumed at Stop, not SessionStart)`)
          }
          if (hookEvent === 'PreToolUse') {
            warnings.push(`${prefix} has "signal" but hook event is "PreToolUse" (signals are consumed at Stop, not PreToolUse)`)
          }
