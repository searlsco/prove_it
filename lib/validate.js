...
const VALID_WHEN_KEYS = ['fileExists', 'envSet', 'envNotSet', 'variablesPresent', 'linesWrittenSinceLastRun', 'linesChangedSinceLastRun', 'sourcesModifiedSinceLastRun', 'toolsUsed', 'sourceFilesEdited']
    ...
    if (!task.when || typeof task.when !== 'object' || Array.isArray(task.when)) {
      ...
      for (const key of Object.keys(task.when)) {
        if (!VALID_WHEN_KEYS.includes(key)) {
          ...
          if (hookType === 'git') {
            warnings.push(`${prefix} has "linesChangedSinceLastRun" but hook type is "git" (this condition requires a Claude Code session for accumulation and will always be 0 in git hooks)`)
          }
