...
const VALID_WHEN_KEYS = ['fileExists', 'envSet', 'envNotSet', 'variablesPresent', 'linesWrittenSinceLastRun', 'sourcesModifiedSinceLastRun', 'toolsUsed', 'sourceFilesEdited']

const KNOWN_TOP_LEVEL_KEYS = [
  'configVersion', 'enabled', 'sources', 'format', 'env', 'hooks', 'runs', 'initSeed', 'fileEditingTools', 'model'
]
const KNOWN_HOOK_KEYS = [
  'type', 'event', 'tasks', 'matcher', 'triggers', 'source'
]
const KNOWN_TASK_KEYS = [
  'name', 'type', 'command', 'prompt', 'promptType', 'when', 'timeout', 'mtime', 'model', 'ruleFile', 'enabled'
]
    ...
    if (typeof cfg.env !== 'object' || cfg.env === null || Array.isArray(cfg.env)) {
      ...
      for (const key of Object.keys(cfg.env)) {
        if (!/^[A-Za-z_][A-Za-z0-9_]*$/.test(key)) {
          errors.push(`env key "${key}" is not a valid environment variable name`)
        }
        if (typeof cfg.env[key] !== 'string') {
    ...
    if (!task.when || typeof task.when !== 'object' || Array.isArray(task.when)) {
      ...
      for (const key of Object.keys(task.when)) {
        if (!VALID_WHEN_KEYS.includes(key)) {
          ...
          if (hookType === 'git') {
            warnings.push(`${prefix} has "linesWrittenSinceLastRun" but hook type is "git" (this condition requires a Claude Code session and will never fire in git hooks)`)
          }
