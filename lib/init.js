...
const { buildConfig, configHash, hasCustomValue } = require('./config')

// Keys whose values are preserved across init/reinit/overwrite when customized
const PRESERVABLE_KEYS = ['sources', 'tests']

/**
 * Copy customized values from an existing config into a fresh config.
 * Returns an object mapping `${key}Preserved` → true for each key that was preserved.
 */
function preserveCustomizations (existing, fresh, overrides = {}) {
  const preserved = {}
  for (const key of PRESERVABLE_KEYS) {
    const resultKey = `${key}Preserved`
    if (overrides[key]) {
      fresh[key] = overrides[key]
      preserved[resultKey] = true
    } else if (hasCustomValue(key, existing)) {
...
const DEFAULT_UI_DESIGN_CONTENT = [
  '# UI Design Rules',
  '',
...
function initProject (repoRoot, options = {}) {
  const { gitHooks = true, defaultChecks = true, autoMergeGitHooks = false, preservedSources, preservedTests } = options
  const overrides = {}
  if (preservedSources) overrides.sources = preservedSources
  if (preservedTests) overrides.tests = preservedTests
  const results = {
    ...
    doneRuleFile: { path: '.claude/rules/done.md', created: false, existed: false },
    uiDesignRuleFile: { path: '.claude/rules/ui-design.md', created: false, existed: false },
  ...
  if (fs.existsSync(teamConfigDst)) {
    ...
    if (existing && existing.initSeed) {
      ...
      if (contentHash === existing.initSeed) {
        ...
        if (contentHash === freshHash) {
          ...
          // Auto-upgrade — preserve customized keys
          const preserved = preserveCustomizations(existing, fresh)
          if (configHash(fresh) === contentHash) {
            // Preserved keys were the only difference; nothing to change
            results.teamConfig.upToDate = true
          } else {
            fresh.initSeed = configHash(fresh)
            writeJson(teamConfigDst, fresh)
            results.teamConfig.upgraded = true
            Object.assign(results.teamConfig, preserved)
          }
        }
      } else {
        // Hash mismatch → user edited. Check if preservable keys are the only customization.
        const fresh = buildConfig({ gitHooks, defaultChecks })
        const preserved = preserveCustomizations(existing, fresh)
    ...
    const preserved = preserveCustomizations(null, cfg, overrides)
  ...
  if (defaultChecks) {
    ...
    const doneRuleFileDst = path.join(repoRoot, '.claude', 'rules', 'done.md')
    if (fs.existsSync(doneRuleFileDst)) {
      results.doneRuleFile.existed = true
    } else {
      ensureDir(path.dirname(doneRuleFileDst))
      fs.writeFileSync(doneRuleFileDst, DEFAULT_DONE_CONTENT)
      results.doneRuleFile.created = true
    }

    const uiDesignRuleFileDst = path.join(repoRoot, '.claude', 'rules', 'ui-design.md')
    if (fs.existsSync(uiDesignRuleFileDst)) {
      results.uiDesignRuleFile.existed = true
    } else {
      ensureDir(path.dirname(uiDesignRuleFileDst))
      fs.writeFileSync(uiDesignRuleFileDst, DEFAULT_UI_DESIGN_CONTENT)
      results.uiDesignRuleFile.created = true
    }
  }
 ...
 * Preserves customized values for PRESERVABLE_KEYS from the existing config.
 */
function overwriteTeamConfig (repoRoot, options = {}) {
  const { gitHooks = true, defaultChecks = true, preservedSources, preservedTests } = options
  const teamConfigDst = path.join(repoRoot, '.claude', 'prove_it', 'config.json')
  ...
  const cfg = buildConfig({ gitHooks, defaultChecks })
  const preserved = preserveCustomizations(existing, cfg, overrides)
  cfg.initSeed = configHash(cfg)
  writeJson(teamConfigDst, cfg)
  // Ensure all keys are present in the return value (false when not preserved)
  for (const key of PRESERVABLE_KEYS) {
    const resultKey = `${key}Preserved`
    if (!(resultKey in preserved)) preserved[resultKey] = false
  }
  return preserved
}
...
function isDefaultDoneFile (filePath) {
  if (!fs.existsSync(filePath)) return false
  try {
    return fs.readFileSync(filePath, 'utf8') === DEFAULT_DONE_CONTENT
  } catch {
    return false
  }
}

function isDefaultUiDesignFile (filePath) {
  if (!fs.existsSync(filePath)) return false
  try {
    return fs.readFileSync(filePath, 'utf8') === DEFAULT_UI_DESIGN_CONTENT
  } catch {
    return false
  }
}
