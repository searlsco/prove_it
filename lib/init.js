...
const { buildConfig, configHash, hasCustomSources, hasCustomTests } = require('./config')
...
function initProject (repoRoot, options = {}) {
  ...
  if (fs.existsSync(teamConfigDst)) {
    ...
    if (existing && existing.initSeed) {
      ...
      if (contentHash === existing.initSeed) {
        ...
        if (contentHash === freshHash) {
          ...
          // Auto-upgrade — preserve custom sources and tests
          if (hasCustomSources(existing)) {
            fresh.sources = existing.sources
          }
          if (hasCustomTests(existing)) {
            fresh.tests = existing.tests
          }
          if (configHash(fresh) === contentHash) {
            // Sources/tests were the only difference; nothing to change
            results.teamConfig.upToDate = true
          } else {
            ...
            if (hasCustomTests(existing)) results.teamConfig.testsPreserved = true
          }
        }
      } else {
        // Hash mismatch → user edited. Check if sources/tests is the only customization.
        // Since sources and tests are always preserved on overwrite, we can auto-upgrade.
        const fresh = buildConfig({ gitHooks, defaultChecks })
        if (hasCustomSources(existing)) fresh.sources = existing.sources
        if (hasCustomTests(existing)) fresh.tests = existing.tests
    ...
    if (preservedTests) cfg.tests = preservedTests
    ...
    if (preservedTests) results.teamConfig.testsPreserved = true
  }
...
function overwriteTeamConfig (repoRoot, options = {}) {
  ...
  if (!sources && hasCustomSources(existing)) sources = existing.sources

  let tests = preservedTests
  if (!tests && hasCustomTests(existing)) tests = existing.tests

  const cfg = buildConfig({ gitHooks, defaultChecks })
  if (sources) cfg.sources = sources
  if (tests) cfg.tests = tests
  cfg.initSeed = configHash(cfg)
  writeJson(teamConfigDst, cfg)
  return { sourcesPreserved: !!sources, testsPreserved: !!tests }
}
