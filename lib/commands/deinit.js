...
function cmdDeinit () {
  ...
  const { isScriptTestStub, isDefaultRuleFile, isDefaultDoneFile, isDefaultUiDesignFile, removeGitHookShim } = require('../init')
  const repoRoot = process.cwd()
  const removed = []
  const skipped = []

  const { deinitStrictProject } = require('../redesign/init')
  const strictResults = deinitStrictProject(repoRoot)
  if (strictResults.removed.length > 0 || strictResults.skipped.length > 0) {
    log('Strict .prove_it project files removed.')
    if (strictResults.removed.length > 0) {
      log('  Removed:')
      for (const f of strictResults.removed) log(`    - ${f}`)
    }
    if (strictResults.skipped.length > 0) {
      log('  Skipped:')
      for (const f of strictResults.skipped) log(`    - ${f.path} (${f.reason})`)
    }
  }
  ...
  const uiDesignRuleFilePath = path.join(repoRoot, '.claude', 'rules', 'ui-design.md')
  if (fs.existsSync(uiDesignRuleFilePath)) {
    if (isDefaultUiDesignFile(uiDesignRuleFilePath)) {
      rmIfExists(uiDesignRuleFilePath)
      removed.push('.claude/rules/ui-design.md')
    } else {
      skipped.push('.claude/rules/ui-design.md (customized)')
    }
  }
