...
const SKILLS = [
  ...
  { name: 'prove-testing-patterns', src: 'prove-testing-patterns.md' },
  { name: 'prove-ui-design', src: 'prove-ui-design.md' }
]
...
/**
 * Return PROVE_IT_SESSION_ID from the environment, or exit 1 with a helpful
 * message. Used by session-scoped commands (cancel, disable, enable) that
 * must be invoked from inside a Claude Code session via `!`.
 */
function requireSessionId (commandName) {
  const sessionId = process.env.PROVE_IT_SESSION_ID
  if (!sessionId) {
    console.error(`prove_it ${commandName}: PROVE_IT_SESSION_ID is not set.`)
    console.error(`This command must be run from within a Claude Code session (via ! prove_it ${commandName}).`)
    process.exit(1)
  }
  return sessionId
}
