...
function parseInitFlags (args) {
  const flags = { gitHooks: true, defaultChecks: true, autoMergeGitHooks: false, overwrite: null, adapters: [] }
  let hasExplicitFlags = false

  for (let i = 0; i < args.length; i++) {
    const arg = args[i]
    if (arg === '--git-hooks') { flags.gitHooks = true; hasExplicitFlags = true } else if (arg === '--no-git-hooks') { flags.gitHooks = false; hasExplicitFlags = true } else if (arg === '--default-checks') { flags.defaultChecks = true; hasExplicitFlags = true } else if (arg === '--no-default-checks') { flags.defaultChecks = false; hasExplicitFlags = true } else if (arg === '--automatic-git-hook-merge') { flags.autoMergeGitHooks = true; hasExplicitFlags = true } else if (arg === '--no-automatic-git-hook-merge') { flags.autoMergeGitHooks = false; hasExplicitFlags = true } else if (arg === '--overwrite') { flags.overwrite = true; hasExplicitFlags = true } else if (arg === '--no-overwrite') { flags.overwrite = false; hasExplicitFlags = true } else if (arg === '--adapter') {
      hasExplicitFlags = true
      const value = args[++i]
      if (!value || value.startsWith('--')) throw new Error('--adapter requires a value')
      flags.adapters.push(...value.split(',').filter(Boolean))
...
function reportStrictInit (results) {
  log('Strict .prove_it initialized.\n')
  for (const created of results.created) log(`  Created: ${created}`)
  for (const updated of results.updated) log(`  Updated: ${updated}`)
  for (const skipped of results.skipped) log(`  Skipped: ${skipped.path} (${skipped.reason})`)
  log(`  Enabled adapters: ${results.adapters.join(', ')}`)
  for (const [label, hook] of Object.entries(results.gitConfigHooks || {})) {
    if (hook.configured) log(`  Configured: git config hook.${hook.hook.name} (${hook.hook.event})`)
    else if (hook.skipped) log(`  Skipped: git config ${label} (${hook.reason})`)
  }
  if (results.manifest.created) log(`  Created: ${results.manifest.path}`)
  else if (results.manifest.existed) log(`  Updated: ${results.manifest.path}`)
  log('')
}
...
async function cmdInit (options = {}) {
  ...
  if (flags.adapters.length > 0) {
    const { initStrictProject } = require('../redesign/init')
    const results = initStrictProject(repoRoot, { adapters: flags.adapters, gitHooks: flags.gitHooks })
    reportStrictInit(results)
    return
  }
  ...
  try {
    ...
    if (results.uiDesignRuleFile.created) {
      log(`  Created: ${results.uiDesignRuleFile.path}`)
    } else if (results.uiDesignRuleFile.existed) {
      log(`  Exists:  ${results.uiDesignRuleFile.path}`)
    }
    ...
    if (results.uiDesignRuleFile.created) {
      todos.push({
        ...
        text: "Customize .claude/rules/ui-design.md with your project's visual design standards"
      })
    }
    ...
    if (scriptsNeedingRecord.length > 0) {
      ...
      for (const label of scriptsNeedingRecord) {
        ...
        log(`\n    # ${label}`)
        log(`    trap 'rc=$?; command -v prove_it >/dev/null 2>&1 && prove_it record --name ${checkName} --result $rc' EXIT`)
      }
    }
    log('')
  } finally {
...
module.exports = { cmdInit, parseInitFlags }
