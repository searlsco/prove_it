...
function suppressRoutineOutput (task) {
  return task?.quiet === true || task?.output === 'failures_only'
}
...
// Log verdict (skip if already logged as BOOM). failures_only suppresses routine
// async worker PASS/SKIP noise but never hides FAIL/BOOM.
const status = result?.skipped ? 'SKIP' : (result?.pass ? 'PASS' : 'FAIL')
const routineSuppressed = suppressRoutineOutput(task)
if (!crashed && !(routineSuppressed && (status === 'PASS' || status === 'SKIP'))) {
  logReview(context.sessionId, context.projectDir, task.name, status, result.reason, null, context.hookEvent)
}

// Log DONE only when useful. For failures_only PASS/SKIP, DONE is also routine
// noise; failed/crashed async work remains visible while awaiting enforcement.
if (!routineSuppressed || status === 'FAIL' || crashed) {
