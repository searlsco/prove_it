 ...
 *   5. Log PENDING to session JSONL (awaiting enforcement)
...
// Apply configEnv to process.env so check functions see it
...
try {
  ...
  logReview(context.sessionId, context.projectDir, task.name, 'CRASH', result.reason, null, context.hookEvent)
}
...
// Log PENDING — signals "result written, awaiting enforcement on next Stop"
logReview(context.sessionId, context.projectDir, task.name, 'PENDING', null, null, context.hookEvent)
...
try {
  ...
  logReview(context.sessionId, context.projectDir, task.name, 'CRASH', `failed to write result: ${e.message}`, null, context.hookEvent)
  process.exit(1)
}
