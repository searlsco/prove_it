...
const TRIGGER_KEYS = [
  'linesChangedSinceLastRun', 'linesWrittenSinceLastRun',
 ...
 *   Phase 2 — Triggers (OR): linesChangedSinceLastRun, linesWrittenSinceLastRun,
 *     sourcesModifiedSinceLastRun, toolsUsed, sourceFilesEdited.
...
function evaluateWhen (when, context, taskName) {
  ...
  if (when.linesChangedSinceLastRun) {
    ...
    if (churn >= when.linesChangedSinceLastRun) {
      anyTriggerPassed = true
    } else {
      lastTriggerReason = `Skipped because only ${churn} of ${when.linesChangedSinceLastRun} lines changed since last run`
    }
  }

  if (when.linesWrittenSinceLastRun) {
    const gross = grossChurnSince(context.rootDir, sanitizeRefName(taskName))
    if (gross >= when.linesWrittenSinceLastRun) {
      anyTriggerPassed = true
    } else {
      lastTriggerReason = `Skipped because only ${gross} of ${when.linesWrittenSinceLastRun} gross lines changed since last run`
    }
  }
  ...
  if (hookEvent === 'PreToolUse' && toolName && fileEditingTools.includes(toolName)) {
    ...
    if (filePath) {
      ...
      if (!relativePath.startsWith('..') && isSourceFile(relativePath, rootDir, cfg.sources)) {
        ...
        // Accumulate gross churn for linesWrittenSinceLastRun tracking
