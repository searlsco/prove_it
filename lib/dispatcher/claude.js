...
function dispatch (event) {
  ...
  if (hookEvent === 'PreToolUse' && toolName === 'EnterPlanMode') {
    if (hasSignalGatedTasks(hooks)) {
      logReview(sessionId, projectDir, 'plan-gate', 'INJECT', 'entering plan mode', null, hookEvent)
      protocol.emitPreToolUse('allow', '', PLAN_SIGNAL_CONTEXT)
    }
    process.exit(0)
  }
  ...
  if (hookEvent === 'PreToolUse' && toolName === 'ExitPlanMode') {
    if (hasSignalGatedTasks(hooks)) {
      ...
      if (!planText.includes('prove_it signal done')) {
        ...
        logReview(sessionId, projectDir, 'plan-gate', 'DENY', 'missing signal step', null, hookEvent)
        protocol.emitPreToolUse('deny', reason)
        process.exit(0)
      }
      logReview(sessionId, projectDir, 'plan-gate', 'ALLOW', null, null, hookEvent)
    }
