...
const TRIGGER_KEYS = [
  'linesWrittenSinceLastRun', 'linesChangedSinceLastRun',
 ...
 *   Phase 2 — Triggers (OR): linesWrittenSinceLastRun, linesChangedSinceLastRun,
 *     sourcesModifiedSinceLastRun, toolsUsed, sourceFilesEdited.
...
function evaluateWhen (when, context, taskName) {
  ...
  if (when.linesWrittenSinceLastRun) {
    ...
    if (churn >= when.linesWrittenSinceLastRun) {
      anyTriggerPassed = true
    } else {
      lastTriggerReason = `Skipped because only ${churn} of ${when.linesWrittenSinceLastRun} lines changed since last run`
    }
  }

  if (when.linesChangedSinceLastRun) {
    const gross = grossChurnSince(context.rootDir, sanitizeRefName(taskName))
    if (gross >= when.linesChangedSinceLastRun) {
      anyTriggerPassed = true
    } else {
      lastTriggerReason = `Skipped because only ${gross} of ${when.linesChangedSinceLastRun} gross lines changed since last run`
    }
  }
  ...
  if (hookEvent === 'PreToolUse' && toolName && fileEditingTools.includes(toolName)) {
    ...
    if (filePath) {
      ...
      if (!relativePath.startsWith('..') && isSourceFile(relativePath, rootDir, cfg.sources)) {
        ...
        // Accumulate gross churn for linesChangedSinceLastRun tracking
