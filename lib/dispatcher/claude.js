...
const { clearSignalOnPass, parseSignalCommand, preserveSignalOnFailure, readSignal, setSignal: setLifecycleSignal } = require('../redesign/signal_lifecycle')
...
async function dispatch (event, _input) {
  ...
  // context-only results (pass/skip); hold failures for the next Stop.
  {
    ...
    for (const ar of asyncResults) {
      ...
      if (settlement.blocked) {
        ...
        if (hookEvent === 'Stop') preserveSignalOnFailure(statePort, sessionId)
  ...
  if (hookEvent === 'Stop') {
    ...
    const signalSettlement = clearSignalOnPass(statePort, sessionId)
