...
async function dispatch (event) {
  ...
  // Uses projectDir (not rootDir) because both it and tool_input file paths come
  // from Claude Code's environment and share the same symlink form.
  if (hookEvent === 'PreToolUse' && sessionId && fileEditingTools.includes(toolName)) {
    ...
    if (bcFilePath && path.isAbsolute(bcFilePath)) {
      const bcPrefix = path.join(projectDir, '.claude', 'prove_it', 'sessions', sessionId, 'backchannel')
