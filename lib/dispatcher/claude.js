...
const SIGNAL_TASK_MARKER = '## Invoke `prove_it signal done`'
const SIGNAL_TASK_BLOCK = `\n${SIGNAL_TASK_MARKER}\n\nAll other work is done. Mark this task complete to signal that work is finished.\n`

/**
 * Find the most recent plan file matching the given plan text and append
 * a signal task checkbox.  Graceful degradation—if plans dir is missing or
 * the file can't be found, we silently skip.
 */
  ...
  try {
    ...
    for (const file of files) {
      ...
      if (content.includes(needle)) {
        if (content.includes(SIGNAL_TASK_MARKER)) return // already appended
        fs.writeFileSync(filePath, content + SIGNAL_TASK_BLOCK, 'utf8')
        return
      }
    }
  } catch {
...
async function dispatch (event) {
  ...
  if (hookEvent === 'TaskCompleted') {
    if (hasSignalGatedTasks(hooks)) {
      ...
      if (/prove_it signal done/i.test(subject)) {
  ...
  if (parallelBatch.length > 0) {
    ...
    for (const pr of parallelResults) {
      ...
      const enforceStatus = pr.result.skipped ? 'SKIP' : 'PASS'
...
module.exports = { dispatch, matchesHookEntry, evaluateWhen, defaultConfig, recordSessionBaseline, writeEnvFile, settleTaskResult, spawnAsyncTask, harvestAsyncResults, cleanAsyncDir, hasSignalGatedTasks, appendSignalTask, forkParallelTask, awaitParallelBatch, killParallelBatch, BUILTIN_EDIT_TOOLS, SIGNAL_TASK_MARKER }
