...
const { SESSION_KEYS, logReview, logCommandResult, pruneOldSessions, recordFileEdit, resetTurnTracking, saveSessionState, loadSessionState, VALID_PHASES, setPhase, writeDispatcherPid, clearDispatcherPid, readCancelSentinel, clearCancelSentinel, readDisabledSentinel } = require('../session')
...
const { createSessionStatePort } = require('../redesign/state_port')
const { parseSignalCommand, readSignal, setSignal: setLifecycleSignal } = require('../redesign/signal_lifecycle')
const { runClaudePreToolUseTaskThroughSharedEngine } = require('../adapters/claude/bridge')
const { dispatchCleanClaudeHookIfEnabled } = require('../adapters/claude/clean_runtime')
const { claudeStopEffectFromVerification, emitClaudePreToolUseEffect, emitClaudeStopEffect } = require('../adapters/claude/effects')
...
function writeEnvFile (vars) {
  ...
  const lines = Object.entries(vars).map(([k, v]) => {
    // Emit as `export KEY=value` so child processes (e.g. `prove_it cancel`
    // invoked via the `!` prefix) inherit the var, not just the shell.
    // Plain `KEY=value` would only set a shell-local var after Claude Code
    // sources the file, so `process.env.KEY` in a Node subprocess would be
    // undefined. See hooks docs: "Standard bash export statements."
    if (/[\n\r"\\]/.test(v)) {
...
async function dispatch (event, _input) {
  ...
  const cleanRoute = dispatchCleanClaudeHookIfEnabled({
    hookEvent,
    input,
    projectDir,
    statePort
  })
  if (cleanRoute.handled) process.exit(0)

  // Check for session-scoped disable sentinel (set by `prove_it disable`).
  // On SessionStart we still inject PROVE_IT_SESSION_ID (so `! prove_it enable`
  // works after resume) and emit a systemMessage reminder — but no
  // additionalContext, no briefings, no tasks. Other events exit silently.
  if (sessionId && readDisabledSentinel(sessionId)) {
  ...
  if (hookEvent === 'PreToolUse' && toolName === 'Bash') {
    ...
    const signalCommand = parseSignalCommand(cmd)
    if (signalCommand) {
      if (signalCommand.valid) {
        setLifecycleSignal(statePort, sessionId, signalCommand.type, signalCommand.message)
        logReview(sessionId, projectDir, 'signal', 'SET', signalCommand.type, null, hookEvent)
    ...
    if (hasSignalGatedTasks(hooks)) {
      ...
      if (SIGNAL_TASK_PATTERN.test(subject)) {
        ...
        if (!existing || existing.type !== 'done') {
          setLifecycleSignal(statePort, sessionId, 'done', null)
  ...
  // context-only results (pass/skip); hold failures for the next Stop.
  {
    ...
    for (const ar of asyncResults) {
      ...
      if (settlement.blocked) {
        ...
        if (hookEvent === 'Stop') {
          emitClaudeStopEffect(claudeStopEffectFromVerification({
            passed: false,
            reason: failMsg,
            statePort,
            sessionId
  ...
  for (const task of matchingTasks) {
    ...
    const sharedPreToolResult = hookEvent === 'PreToolUse'
  ...
  if (hookEvent === 'PreToolUse') {
    ...
    const stopEffect = claudeStopEffectFromVerification({
      passed: true,
      reason: `prove_it: ${summary}`,
      statePort,
      sessionId
    })
    emitClaudeStopEffect(stopEffect)
    context._stopSignalSettlement = stopEffect.signalLifecycle
  } else {
    protocol.emit(hookEvent, protocol.passDecision(hookEvent),
      `prove_it: ${summary}`)
  }
  ...
  // After successful Stop: checkpoint git HEAD, reset phase when done clears, reset turn tracking
  if (hookEvent === 'Stop') {
    const head = gitHead(rootDir)
    if (head) saveSessionState(sessionId, SESSION_KEYS.LAST_STOP_HEAD, head)
    const signalSettlement = context._stopSignalSettlement
    if (signalSettlement?.signal?.type === 'done') setPhase(sessionId, 'unknown')
