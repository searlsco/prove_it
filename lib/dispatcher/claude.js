...
const { resolveTestRoot, getLatestMtime, loadRunData, saveRunData, runResult, isTestCommand } = require('../testing')
...
const { logReview, getAsyncDir, pruneOldSessions, recordFileEdit, getFileEdits, resetTurnTracking, saveSessionState, loadSessionState, VALID_SIGNALS, setSignal, getSignal, clearSignal, incrementConsecutiveUntestedEditCount, resetConsecutiveUntestedEditCount } = require('../session')
...
const { SIGNAL_PLAN_MARKER, SIGNAL_TASK_PATTERN, TDD_MARKER, TDD_BLOCK, detectLastNumberedHeading, buildSignalBlock, findPlanFile, appendPlanBlock } = require('../plan')

const BUILTIN_EDIT_TOOLS = ['Edit', 'MultiEdit', 'Write', 'NotebookEdit']

/**
 * Find the most recent plan file matching the given plan text and inject
 * TDD guidance and signal step. Graceful degradation—if plans dir is
 * missing or the file can't be found, we silently skip.
 */
function injectPlanBlocks (toolInput, hasSignalGated) {
  try {
    ...
    // Always inject TDD block after the title
    ...
    // Inject signal block before verification (if signal-gated tasks exist)
    if (hasSignalGated) {
      let content
...
async function dispatch (event) {
  ...
  if (hookEvent === 'PreToolUse' && toolName && fileEditingTools.includes(toolName)) {
    ...
    if (filePath) {
      ...
      if (!relativePath.startsWith('..') && isSourceFile(relativePath, rootDir, cfg.sources)) {
        ...
        // Test-first counter tracking
        if (isTestFile(relativePath, rootDir, cfg.tests)) {
          resetConsecutiveUntestedEditCount(sessionId)
        } else {
          incrementConsecutiveUntestedEditCount(sessionId)
        }
      }
    }
  }
  ...
  if (hookEvent === 'PreToolUse' && toolName === 'Bash') {
    ...
    // Test-run detection: reset counter when a test command is detected
    if (isTestCommand(cmd, cfg.testCommands)) {
      resetConsecutiveUntestedEditCount(sessionId)
    }
  }
  ...
  if (hookEvent === 'PreToolUse' && toolName === 'ExitPlanMode') {
    injectPlanBlocks(input.tool_input, hasSignalGatedTasks(hooks))
  ...
  const context = {
    ...
    configMaxAgentTurns: cfg.maxAgentTurns || null,
  ...
  if (hookEvent === 'Stop') {
    ...
    for (const ar of asyncResults) {
      ...
      if (ar.data.task.type === 'script' && ar.data.result.pass && !ar.data.result.skipped) {
        ...
        if (/test/i.test(ar.data.task.name)) {
          resetConsecutiveUntestedEditCount(sessionId)
        }
      }
  ...
  for (const entry of matchingEntries) {
    ...
    for (const task of tasks) {
      ...
      if (task.type === 'script' && result.pass && !result.skipped) {
        ...
        // Reset test-first counter when a test task passes
