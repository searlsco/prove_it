...
const { gitRoot, gitHead, gitStatusHash, churnSinceRef, sanitizeRefName, advanceChurnRef } = require('../git')
...
function evaluateWhen (when, context, taskName) {
  ...
  if (when.linesWrittenSinceLastRun) {
    ...
    if (churn < when.linesWrittenSinceLastRun) return `Skipped because only ${churn} of ${when.linesWrittenSinceLastRun} lines changed since last run`
  }
...
module.exports = { dispatch, matchesHookEntry, evaluateWhen, defaultConfig, recordSessionBaseline, writeEnvFile, BUILTIN_EDIT_TOOLS }
