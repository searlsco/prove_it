...
function dispatch (event) {
  ...
  if (hookEvent === 'SubagentStart') {
    ...
    const hasSignalGated = hasSignalGatedTasks(hooks)
    if (agentType === 'Plan' && hasSignalGated) {
      contextParts.push(PLAN_SIGNAL_CONTEXT)
    }

    const additionalContext = contextParts.join('\n') || null
    logReview(sessionId, projectDir, 'subagent-start', additionalContext ? 'INJECT' : 'NOOP',
      `agent_type=${agentType} signalGated=${hasSignalGated} contextLen=${additionalContext ? additionalContext.length : 0}`,
      null, hookEvent)
