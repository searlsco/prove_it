...
const { logReview, getAsyncDir, recordFileEdit, getFileEdits, resetTurnTracking, saveSessionState } = require('../session')
...
function settleTaskResult (task, result, hookEvent, settlCtx, outputs, contextParts, systemMessages) {
  ...
  if (!result.pass && !result.skipped) {
    ...
    advanceChurnRef(task, false, hookEvent, rootDir, sources)
  ...
  // PASS
  advanceChurnRef(task, true, hookEvent, rootDir, sources)
 ...
 * Reads and deletes result files atomically (read → unlink → return).
 */
...
function dispatch (event) {
  ...
  if (hookEvent === 'Stop') {
    ...
    for (const ar of asyncResults) {
      const settlement = settleTaskResult(ar.task, ar.result, hookEvent, settlCtx, outputs, contextParts, systemMessages)
      ...
      logReview(sessionId, projectDir, ar.taskName, 'ENFORCED:PASS', ar.result.reason, null, hookEvent)
    }
  }
...
module.exports = { dispatch, matchesHookEntry, evaluateWhen, defaultConfig, recordSessionBaseline, writeEnvFile, settleTaskResult, spawnAsyncTask, harvestAsyncResults, cleanAsyncDir, BUILTIN_EDIT_TOOLS, PREREQUISITE_KEYS, TRIGGER_KEYS }
