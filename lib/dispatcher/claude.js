...
const PLAN_SIGNAL_CONTEXT = `When this plan involves implementing code changes, include as the final
verification step:

  prove_it signal done -m "<brief summary of what was implemented>"

This triggers heavyweight verification checks (full test suite, code review)
at the session's next stopping point. Omit this step for plans that are pure
research, exploration, or trivial one-line fixes.`
...
function dispatch (event) {
  ...
  try {
    ...
    if (event === 'SubagentStart') {
      protocol.emitSubagentStart({ additionalContext: failMsg })
    } else {
  ...
  try {
    ...
    if (hookEvent === 'SubagentStart') {
      protocol.emitSubagentStart({ additionalContext: e.message })
    } else {
  ...
  if (matchingEntries.length === 0 && hookEvent !== 'SubagentStart') {
    process.exit(0)
  }
  ...
  if (hookEvent === 'SubagentStart') {
    // Combine task outputs into context (settleTaskResult puts them in outputs)
    contextParts.push(...outputs.filter(Boolean))

    const agentType = input.agent_type || ''
    if (agentType === 'Plan') {
      contextParts.push(PLAN_SIGNAL_CONTEXT)
    }

    const additionalContext = contextParts.join('\n') || null
    protocol.emitSubagentStart({ additionalContext })
    process.exit(0)
  }
...
module.exports = { dispatch, matchesHookEntry, evaluateWhen, defaultConfig, recordSessionBaseline, writeEnvFile, settleTaskResult, spawnAsyncTask, harvestAsyncResults, cleanAsyncDir, BUILTIN_EDIT_TOOLS, PLAN_SIGNAL_CONTEXT }
