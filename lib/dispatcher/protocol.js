...
function emitPreToolUseContext (additionalContext, opts = {}) {
  // Without permissionDecision, only additionalContext reaches the agent.
  // permissionDecisionReason is invisible on the pass path (proven by ad hoc test).
  const output = {
    hookSpecificOutput: {
      hookEventName: 'PreToolUse'
    }
