...
Issue #34 implementation notes:

- The clean `claude` profile retains `.prove_it` config guarding, test-first Pre Tool guidance, ExitPlanMode assumption verification, fast tests, Done-gated full tests, Done review, Stuck approach review, coverage review, testing-antipattern review, and the legacy pre-commit full-test default in strict profile data.
- Session Start briefing is retained as the clean runtime's built-in methodology/context renderer for now, not as the legacy `libexec/briefing` script, because that script reads Legacy Runtime config.
- Claude skill-backed reviewers are represented in strict reviewer tasks as `prompt: "skill:<skill-name>"`; the Claude reviewer backend maps that to Claude skill execution without reintroducing legacy `promptType` fields in `.prove_it` config.
- UI design review is intentionally deferred until strict `when.fileExists` (or an equivalent rule-file capability) exists, because the legacy default depended on `.claude/rules/ui-design.md` presence.
- Legacy `inject-tdd-plan` plan-file mutation is intentionally deferred until strict script task parameters or a clean built-in plan guidance capability exists; Claude signal/phase plan block injection remains adapter behavior from the clean phase/plan slice.
- Global install-only defaults such as `enabled`, `maxAgentTurns`, and `taskEnv.TURBOCOMMIT_DISABLED` remain deferred to clean global config/install work; they are not project profile defaults.
