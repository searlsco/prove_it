...
const ADAPTER_CONFIG_KEYS = {
  pi: new Set(['enabled']),
  claude: new Set(['enabled', 'file_editing_tools']),
  codex: new Set(['enabled'])
}
...
function validateAdapters (adapters, filePath) {
  ...
  for (const [name, adapter] of Object.entries(adapters)) {
    ...
    assertKnownKeys(adapter, ADAPTER_CONFIG_KEYS[name], `adapters.${name}`, filePath)
    ...
    if (name === 'claude' && adapter.file_editing_tools !== undefined) {
      validateStringArray(adapter.file_editing_tools, 'adapters.claude.file_editing_tools', filePath)
    }
    normalized[name] = clone(adapter)
  }
  return normalized
}
