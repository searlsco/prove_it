...
function launchAsyncTask (taskName, task, context) {
  ...
  const handle = invokeTaskPortMethod(lifecyclePort, ['launchBackgroundTask', 'startBackgroundTask', 'launchAsyncTask'], lifecycleTaskContext(taskName, task, context), context)
...
function startParallelTask (taskName, task, context) {
  const lifecyclePort = resolveLifecyclePortForTask(task, context)
  const handle = invokeTaskPortMethod(lifecyclePort, ['startParallelTask', 'forkParallelTask'], lifecycleTaskContext(taskName, task, context), context)
...
function settleDeferredParallelHandle (handle, context) {
  const result = runTaskForResult(handle.taskName, handle.task, context)
